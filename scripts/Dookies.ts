...
async function initContracts() {
    ...
    token = await tokenFactory.deploy("GHOST", "GHO")
    ...
    console.log(`The balance of ${accounts[1].address} is ${bal} GHO`)

    const contractFactory = await ethers.getContractFactory("Dookies")
    dookies = await contractFactory.deploy(token.address)
